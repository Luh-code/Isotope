...
//type MsgAction = Box<dyn Fn(&[&dyn std::any::Any]) -> Box<dyn std::any::Any>>;
type MsgAction = Box<dyn Fn()>;
...
pub struct Editor {
  ...
  custom_msg_actions: HashMap<String, MsgAction>,
}
  ...
  fn new(flags: EditorFlags) -> (Self, Command<Message>) {
    ...
    (
      Editor {
        ...
        custom_msg_actions: HashMap::new(),
      },
      Command::none(),
    )
  }
  ...
  fn update(&mut self, message: Message) -> Command<Message> {
    match message {
      ...
      Message::Custom(alias) => {
        match self.custom_msg_actions.get(&alias) {
          Some(action) => action(),
          None => println!("The action '{}' is not defined!", alias),
        }
      }
    }
